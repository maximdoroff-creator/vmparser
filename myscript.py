...
if 'auth' not in st.session_state:
    ...
    if query_user:
        user_match = next((x for x in st.session_state.db["users"] if x["login"] == query_user), None)
        if user_match:
...
if not st.session_state.get('auth'):
    ...
    cs2.markdown(f'<div class="status-card"><div class="status-label">Telegram Status</div><div style="font-size:18px;">{tg_stat_html}</div></div>', unsafe_allow_html=True)
