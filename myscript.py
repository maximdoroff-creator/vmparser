...
st.markdown("""
    ...
    /* Простые темные кнопки (Как в ExceptionModels) */
...
if not st.session_state.get('auth'):
    ...
    with col_login:
        ...
        if st.button("ENTER"):
            # Принудительно перечитываем базу перед входом
            st.session_state.db = load_db()
    ...
    with tabs[1]: # АККАУНТ
        ...
        if u["role"] == "Админ": st.success(f"✅ Админ-аккаунт активен: {u.get('tg_name')}")
            ...
            if st.button("Прислать код"):
                ...
                st.session_state.tmp_c, st.session_state.tmp_h = run_async(get_c())
                st.info("Код отправлен!")
            
            code = st.text_input("Код")
            if st.button("АКТИВИРОВАТЬ"):
                if 'tmp_c' not in st.session_state:
                    st.error("Ошибка: Сначала нажмите 'Прислать код'")
                else:
                    async def finish():
