...
if not st.session_state.get('auth'):
    ...
    cs2.markdown(f'<div class="stat-card"><div class="stat-label">Telegram Status</div><div style="font-size:18px;">{tg_html}</div></div>', unsafe_allow_html=True)
    ...
    with tabs[0]: # СБОР
        ...
        if not sess_str: st.warning("⚠️ Подключите Telegram во вкладке АККАУНТ")
        else:
    ...
    with tabs[1]: # АККАУНТ
        ...
        if u["role"] == "Админ": st.success(f"✅ Админ-аккаунт активен: {u.get('tg_name')}")
            ...
            if st.button("Прислать код"):
                ...
                st.session_state.tmp_c, st.session_state.tmp_h = run_async(get_c()); st.info("Код отправлен!")
            
            t_code = st.text_input("Код подтверждения")
            if st.button("АКТИВИРОВАТЬ"):
                async def finish_auth():
                    await st.session_state.tmp_c.sign_in(phone, t_code, phone_code_hash=st.session_state.temp_hash if 'temp_hash' in st.session_state else st.session_state.tmp_h)
                    me = await st.session_state.tmp_c.get_me()
                    s_str = st.session_state.tmp_c.session.save()
                    uname = f"@{me.username}" if me.username else f"{me.first_name}"
                    for db_u in st.session_state.db["users"]:
                        if db_u["login"] == u["login"]: db_u["session"], db_u["tg_name"] = s_str, uname
                    save_db(st.session_state.db)
                run_async(finish_auth()); st.rerun()
